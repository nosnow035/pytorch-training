...
if __name__ == "__main__":
 ...
 """ problem 1 """
 _tensor = torch.tensor(data, dtype=float) # float型のTensor型への変換
 print("===== problem 1 =====")
 print(repr(_tensor.size())) #配列の大きさ.sizeで

 """ problem 2 """
 permuted_tensor = torch.permute(_tensor, (2, 0, 1)) # 軸入れ替え　　torch.permute（）
 print("===== problem 2 =====")
 ...
 """ problem 4 """
 mean_method1 = sum_permuted_tensor.mean(dim=1) # 1軸での平均値を算出　mean 平均
 print("===== problem 4 =====")
 print(mean_method1)

 """ problem 5 """
 tmp_sum = sum_permuted_tensor.sum(dim=1) # 1軸での合計値を算出
 num_classmates = sum_permuted_tensor.size(dim=1) # 1軸での配列長を取得　size配列長さ取得
